...
// MODIFICACIÓN: LEFT JOIN para soportar Empleados y Clientes
$sql_select = "SELECT 
                    a.id AS id_asignacion, a.fecha_entrega, a.fecha_devolucion,
                    CASE WHEN a.fecha_devolucion IS NULL THEN 'Activa' ELSE 'Finalizada' END AS estado_asignacion,
...
if (!empty($filtro_estado)) {
    if ($filtro_estado == 'Activa')
        $where_clauses[] = "a.fecha_devolucion IS NULL";
    elseif ($filtro_estado == 'Finalizada')
        $where_clauses[] = "a.fecha_devolucion IS NOT NULL";
}
if (!empty($filtro_fecha_desde)) {
    $where_clauses[] = "a.fecha_entrega >= ?";
    $types .= "s";
        ...
        <form action="asignaciones.php" method="GET">
            <div class="row g-3">
                <?php if ($id_sucursal_usuario === null): ?>
                    <div class="col-md-4"><label class="form-label">Sucursal</label><select
                ...
                <?php endif; ?>
                <div class="col-md-4"><label class="form-label">Empleado</label><select
                        class="form-select form-select-sm" name="empleado">
                ...
                <div class="col-md-3"><label class="form-label">Fecha Entrega (Hasta)</label><input type="date"
                        class="form-control form-control-sm" name="fecha_hasta"
                        value="<?php echo htmlspecialchars($filtro_fecha_hasta); ?>"></div>
                <div class="col-md-3 d-flex align-items-end"><button type="submit"
                        class="btn btn-primary btn-sm me-2">Filtrar</button><a href="asignaciones.php"
                        class="btn btn-secondary btn-sm">Limpiar</a></div>
            </div>
        </form>
        <hr>
                            ...
                            <tr>
                                <td>
                                    ...
                                    if (!empty($asignacion['emp_nombres'])) {
                                        echo '<i class="bi bi-person-badge text-primary" title="Empleado"></i> ' . htmlspecialchars($asignacion['emp_apellidos'] . ', ' . $asignacion['emp_nombres']);
                                    } elseif (!empty($asignacion['cli_nombres'])) {
                                        echo '<i class="bi bi-people-fill text-success" title="Cliente"></i> ' . htmlspecialchars($asignacion['cli_apellidos'] . ', ' . $asignacion['cli_nombres']);
                                    } else {
                                        echo '<span class="text-muted">Desconocido</span>';
                                    }
                                    ?>
                                </td>
                                <td><?php echo htmlspecialchars($asignacion['codigo_inventario'] . ' (' . $asignacion['marca_nombre'] . ')'); ?>
                                </td>
                                <td><?php echo date('d/m/Y', strtotime($asignacion['fecha_entrega'])); ?></td>
                                <td><?php echo $asignacion['fecha_devolucion'] ? date('d/m/Y', strtotime($asignacion['fecha_devolucion'])) : '<span class="text-muted">---</span>'; ?>
                                </td>
                                <td><span
                                        class="badge <?php echo $asignacion['estado_asignacion'] === 'Activa' ? 'bg-success' : 'bg-secondary'; ?>"><?php echo htmlspecialchars($asignacion['estado_asignacion']); ?></span>
                                </td>
                                <td>
                                    <?php if ($asignacion['acta_firmada_path']): ?>
                                        <a href="descargar_acta.php?id_asignacion=<?php echo $asignacion['id_asignacion']; ?>&tipo=entrega"
                                            target="_blank" class="btn btn-info btn-sm" title="Ver Acta Entrega"><i
                                                class="bi bi-file-earmark-pdf-fill"></i></a>
                                    <?php else: ?>
                                        <a href="asignacion_subir_acta.php?id=<?php echo $asignacion['id_asignacion']; ?>"
                                            class="btn btn-outline-primary btn-sm" title="Subir Acta Entrega"><i
                                ...
                                <td>
                                    <?php if ($asignacion['estado_asignacion'] === 'Finalizada'): ?>
                                        <?php if ($asignacion['acta_devolucion_path']): ?>
                                            <a href="descargar_acta.php?id_asignacion=<?php echo $asignacion['id_asignacion']; ?>&tipo=devolucion"
                                                target="_blank" class="btn btn-info btn-sm" title="Ver Acta Devolución"><i
                                                    class="bi bi-file-earmark-pdf-fill"></i></a>
                                        <?php else: ?>
                                            <a href="asignacion_subir_acta_devolucion.php?id=<?php echo $asignacion['id_asignacion']; ?>"
                                                class="btn btn-outline-danger btn-sm" title="Subir Acta Devolución"><i
                                                    class="bi bi-upload"></i></a>
                                        <?php endif; ?>
                                    <?php else: ?>
                                ...
                                <td>
                                    <div class="btn-group" role="group">
                                        ...
                                        <?php else: ?>
                                            <a href="asignacion_detalle_devolucion.php?id=<?php echo $asignacion['id_asignacion']; ?>"
                                                class="btn btn-primary btn-sm" title="Ver Detalle Devolución"><i
                                                    class="bi bi-eye"></i></a>
                                        <?php endif; ?>
                                    </div>
                                </td>
...
<script>
    $(document).ready(function () {
        ...
        if ($.fn.DataTable.isDataTable('#tabla-asignaciones')) { $('#tabla-asignaciones').DataTable().destroy(); }

        // Detectar si la tabla está vacía
        var tablaVacia = $('#tabla-asignaciones tbody tr').length === 0;
